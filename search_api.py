...
def apiReq(search_list_cl=['PIM','DR'], search_oper_cl='AND',search_type_cl='D',time_min=0,time_max=30000000000,search_list_ca=[''],search_oper_ca='OR'):


    if 'APIREQ' not in glob.glob('*'):
        os.mkdir('APIREQ')
    ...
    time.sleep(.2)
    CS.searchSend('api',search_list_cl,search_oper_cl,search_type_cl,None,time_min,time_max)
    time.sleep(.2)
    ...
    CS.searchSend('api',None,None,'W',None,None,None)
    time.sleep(.2)
    CS.searchSend('api',search_list_ca,search_oper_ca,'C',api_file,None,None)
    time.sleep(1.5)
    ...
    try:
        ...
        error=json.dumps("Can't find %s"%(api_file))
        return error
